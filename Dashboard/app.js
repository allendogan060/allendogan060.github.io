...
function renderProducts() {
  ...
  $("view").innerHTML = `
    <div class="page-tools">
      ...
      <button class="primary" type="button" data-action="add-product">+ Produkt</button>
    </div>
...
function renderOrderModal(table) {
  const categories = [...new Set(app.data.products.map((product) => product.category))];
  ...
  openModal({
    ...
    footer: `
      ...
      <button class="primary" type="button" data-modal-action="submit-order" ${app.orderCart.length ? "" : "disabled"}>Bonieren · ${formatCurrency(cartTotal)}</button>
    `
  });
}
...
function addCart(productID) {
  ...
  const existing = app.orderCart.find((item) => item.productID === product.id);
